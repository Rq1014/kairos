...
DELETE FROM `question_knowledge_point` WHERE question_code = 'q-shared-eigen-1';
DELETE FROM `question_scope` WHERE question_code = 'q-shared-eigen-1';
DELETE FROM `exam_paper_scope` WHERE paper_code = 'p-todai-2024-math';
DELETE FROM `question` WHERE code LIKE 'q-todai-2024-math-%';
DELETE FROM `question` WHERE code = 'q-shared-eigen-1';
...
-- 试卷归属(单归属: 东大 info-sci/cs/math/2024)
...
-- 散题(不挂卷)多归属样例: 同题既属 东大 info-sci/cs/math/2024, 又属 京大 informatics/ii/math/2022
INSERT INTO `question`
  (`code`, `paper_code`,
   `question_no`, `title`, `order_index`, `content_blocks`, `body_text`,
   `difficulty_label`, `difficulty_level`, `crowd_difficulty_rate`, `status`)
VALUES
  ('q-shared-eigen-1', NULL,
   '第1问', '固有值分解(跨校共享样例)', 1, NULL,
...
VALUES
  ...
  ('q-shared-eigen-1', 'kyodai', 'informatics', 'ii', 'math', 2022, 1);
...
INSERT INTO `question_knowledge_point` (`question_code`, `knowledge_point`) VALUES
  ...
  ('q-shared-eigen-1', '线性代数'),
  ('q-shared-eigen-1', '固有值');
